...
When(
    'User1 request the QR code from the Web Application to allow logging in the Mobile App with {string} and {string} and {string} company name',
    async (username, password, companyName) => {
...
When(
    'User1 go to Contact tab and search for user 2 {string}',
    async (searchInput) => {
...
When(
    'User1 reply {string} with {string}',
    async (targetMessage, replyMessage) => {
        mobilePage.replyToLatestMessageSent(targetMessage, replyMessage);
    }
);

// Then('User2 login to the Web Application and received message and reply message', async () => {
